...
# Treatment #
trt.propensity <- 9.1 + 1.3*w + - 1.9*r + .9*rnorm(N) # Error term must be multivariate normal with main equation. Make sure there are enough Ts!
...
T.probit <- glm(T ~ x1 + x2 + r, family = binomial(link = "probit"))
summary(T.probit)
...
# My Summary: it works, but parameter coefficients other than the treatment are distorted.
## Can they be recovered? Remember that the regression model is, by definition, mis-specified.
### Here we have an instrument. How does this compare to standard instrumental variable regression?
### What if treatment interacts with some other variable?
### That is, how do you do conditional inference in the face of selection bias. Does it proceed 
### in the same manner?
