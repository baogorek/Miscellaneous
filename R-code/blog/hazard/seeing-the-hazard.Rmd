...
Beyond R itself, there's nothing to install.
...
* covariate values `x1` and `x2` (the values of $\beta$ corresponding to `x1` and `x2`, 
...
we want the sinusoidal back. As discussed, we have to settle first
...
there's more than one way to accomplish this, the parametric spline-based technique used
below is fast
and easily allows for interpolation onto an equally spaced grid. The `df`
...
plot(haz_df$H ~ haz_df$t, main = "The cumulative hazard with spline fit",
     ylab = "H(t)", xlab = "t", cex.lab = 1.4)
lines(surv_spline$fitted ~ haz_df$t, col = "red", lwd = 1.2)
```
