# THis exercise is based on the paper by Neal, "Sampling Methods For Dirichlet Process Mixture Models" #
...
for(i in 1:N){
  ...
  if( runif(1) < alpha/(i-1+alpha) ){ 
    theta[i] <- rnorm(1, mean = 0, sd = 3) # draw from the base distribution
    cat("Current Index: ", i,  " \n Previous Index: NA - Using G0 to generate another atom: ", theta[i], " \n" )
  }
  else{
    previous.index <- round(.5 + runif(1)*(i-1))
    cat("Current Index: ", i,  " \n Previous Index: ", previous.index, "with value: ", theta[previous.index], "\n" )
...
### Making the DP continuous with the Mixtures. The distribution of the data given the DP draws is chosen 
### to be N(theta,1) for conjugacy and convenience ###

y <- rep(0, times = N)

for(i in 1:N){
    y[i] <- rnorm(1, mean = theta[i], sd = 1)
}
